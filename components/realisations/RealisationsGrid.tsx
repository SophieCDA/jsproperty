...
export function RealisationsGrid({ properties, className }: RealisationsGridProps) {
  ...
  return (
    <div ref={gridRef} className={cn("grid md:grid-cols-2 xl:grid-cols-3 gap-6 lg:gap-8", className)}>
      {properties.map((property, index) => (
        ...
        >
          <RealisationCard property={property} />
        </div>
      ))}
    </div>
  );
  ...
  return (
    <div className="flex flex-col items-center justify-center py-20 text-center">
      ...
      <p className="font-sans text-base text-gris-noble max-w-md">
        Modifiez vos critères de recherche pour découvrir l'ensemble de nos réalisations sur la Côte d'Azur.
      </p>
    </div>
  );
}